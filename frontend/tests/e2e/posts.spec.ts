...
const TEST_POST = {
  title: 'E2Eテスト記事',
  content: 'これはE2Eテスト用の記事コンテンツです。Playwrightを使用してテストしています。',
};
...
test.describe('Posts Management E2E Tests', () => {
  ...
  test.describe('Admin Posts Management', () => {
    ...
    test('should create new post successfully', async ({ page }) => {
      ...
      await expect(page.locator('[role="alert"]:has-text("記事が作成されました")')).toBeVisible();

      // 記事一覧に戻ることを確認
      await page.waitForURL(DASHBOARD_URL);

      // 作成した記事が一覧に表示されることを確認
      await expect(page.locator(`td:has-text("${TEST_POST.title}")`)).toBeVisible();
    });
    ...
    test('should update post successfully', async ({ page }) => {
      ...
      // 成功メッセージの確認（Alert component）
      await expect(page.locator('[role="alert"]:has-text("記事が更新されました")')).toBeVisible();

      // 記事一覧に戻ることを確認
      await page.waitForURL(DASHBOARD_URL);

      // 更新された記事が一覧に表示されることを確認
      await expect(page.locator(`td:has-text("${UPDATED_POST.title}")`)).toBeVisible();
    });
    ...
    test('should delete post successfully', async ({ page }) => {
      ...
      await expect(page.locator('.delete-confirmation')).toContainText('削除しますか？');

      // 削除を確定
      await page.click('button:has-text("削除する")');

      // 成功メッセージの確認（Alert component）
      await expect(page.locator('[role="alert"]:has-text("記事が削除されました")')).toBeVisible();

      // 削除された記事が一覧から消えることを確認
      await expect(page.locator(`td:has-text("${TEST_POST.title}")`)).not.toBeVisible();
    });
    ...
    test('should toggle post publication status', async ({ page }) => {
      ...
      await expect(postRow.locator('.status-badge:has-text("下書き")')).toBeVisible();
      ...
      // 公開ステータスの確認
      await expect(postRow.locator('.status-badge:has-text("公開")')).toBeVisible();
    });
  ...
  test.describe('Post Form Validation', () => {
     ...
     * 記事作成フォームのバリデーションテスト
     */
    test('should validate required fields in post creation form', async ({ page }) => {
      await loginAsAdmin(page);
      ...
      await expect(page.locator('p.text-sm.text-red-600:has-text("タイトルは必須です")')).toBeVisible();
      await expect(page.locator('p.text-sm.text-red-600:has-text("内容は必須です")')).toBeVisible();
    });
    ...
    test('should validate character limits', async ({ page }) => {
      ...
      // バリデーションエラーメッセージの確認（React Hook Form）
      await expect(page.locator('p.text-sm.text-red-600:has-text("タイトルは255文字以内で入力してください")')).toBeVisible();
    });
  });
  ...
  test.describe('Integration Tests', () => {
     ...
     */
    test('should display admin-created post on public site', async ({ page }) => {
      await loginAsAdmin(page);
      ...
      await expect(page.locator('h1:has-text("Public Test Post")')).toBeVisible();
      await expect(page.locator('text="This post should appear on the public site."')).toBeVisible();
    });
