...
const mockHandleApiError = require('@/lib/api').handleApiError as jest.MockedFunction<any>;
...
describe('usePosts', () => {
  ...
  describe('fetchAdminPosts', () => {
    it('should fetch admin posts successfully', async () => {
      const mockResponse = {
        data: {
          data: [
            { id: 1, title: 'Admin Post 1', content: 'Content 1', status: 'published' },
            { id: 2, title: 'Admin Post 2', content: 'Content 2', status: 'draft' },
          ],
          meta: {
            current_page: 1,
            last_page: 1,
      ...
      expect(mockApi.get).toHaveBeenCalledWith('/admin/posts', { params: undefined });
      expect(result.current.posts).toEqual(mockResponse.data.data);
      expect(result.current.pagination).toEqual({
        currentPage: 1,
        lastPage: 1,
  ...
  describe('fetchAdminPost', () => {
    it('should fetch single admin post successfully', async () => {
      const mockResponse = {
        data: {
          data: { id: 1, title: 'Admin Post', content: 'Content', status: 'published' },
        },
      };
  ...
  describe('createPost', () => {
    it('should create post successfully', async () => {
      ...
      const postData = { title: 'New Post', content: 'New Content', status: 'draft' as const };
      ...
      expect(result.current.loading).toBe(false);
      expect(result.current.error).toBeNull();
      expect(createResult).toEqual({ success: true, post: mockResponse.data.data });
    });
    ...
    it('should handle creation errors', async () => {
      ...
      let createResult;
      await act(async () => {
        createResult = await result.current.createPost({ title: 'Test', content: 'Content', status: 'draft' });
      });

      expect(result.current.loading).toBe(false);
      expect(result.current.error).toBeTruthy();
      expect(createResult).toEqual({ success: false, error: expect.any(String) });
    });
  });
  ...
  describe('updatePost', () => {
    it('should update post successfully', async () => {
      ...
      const updateData = { id: 1, title: 'Updated Post', content: 'Updated Content', status: 'published' as const };
      ...
      expect(result.current.loading).toBe(false);
      expect(result.current.error).toBeNull();
      expect(updateResult).toEqual({ success: true, post: mockResponse.data.data });
    });
  });
