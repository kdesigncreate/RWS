...
const mockSecureApiClient = secureApiClient as jest.Mocked<
  typeof secureApiClient
>;
...
describe("API Endpoints", () => {
  ...
  describe("queryKeys", () => {
    it("should generate correct query keys for posts", () => {
      ...
      expect(queryKeys.posts.list({ status: "published" })).toEqual([
        "posts",
        "list",
        { status: "published" },
      ]);
      ...
      expect(queryKeys.posts.search("test")).toEqual([
        "posts",
        "search",
        "test",
      ]);
  ...
  describe("authApi", () => {
    it("should call login endpoint correctly", async () => {
      ...
      expect(mockSecureApiClient.post).toHaveBeenCalledWith(
        "/auth/login",
        loginData,
      );
    ...
    it("should call logout endpoint correctly", async () => {
      mockSecureApiClient.post.mockResolvedValue({
    ...
    it("should call profile endpoints correctly", async () => {
      ...
      mockSecureApiClient.put.mockResolvedValue({
        data: { updated: true },
      } as any);
      await authApi.updateProfile(updateData);
      expect(mockSecureApiClient.put).toHaveBeenCalledWith(
        "/auth/profile",
        updateData,
      );
    });
  ...
  describe("postsApi", () => {
    it("should call posts endpoints correctly", async () => {
      ...
      expect(mockSecureApiClient.get).toHaveBeenCalledWith(
        "/posts?page=1&limit=10&status=published",
      );
      ...
      expect(mockSecureApiClient.put).toHaveBeenCalledWith("/posts/123", {
        title: "Updated Post",
        content: "Updated Content",
        status: "published",
        csrfToken: "csrf-token",
    ...
    it("should handle search correctly", async () => {
      ...
      const call =
        mockSecureApiClient.get.mock.calls[
          mockSecureApiClient.get.mock.calls.length - 1
        ][0];
  ...
  describe("commentsApi", () => {
    it("should handle comment operations correctly", async () => {
      ...
      expect(mockSecureApiClient.get).toHaveBeenCalledWith(
        "/posts/123/comments?page=1&limit=5",
      );
  ...
  describe("uploadApi", () => {
    it("should handle file uploads correctly", async () => {
      ...
      expect(mockSecureApiClient.upload).toHaveBeenCalledWith(
        "/upload",
        expect.any(FormData),
      );
  ...
  describe("securityApi", () => {
    it("should handle security reporting correctly", async () => {
      ...
      expect(mockSecureApiClient.get).toHaveBeenCalledWith(
        "/security/events?page=1&limit=20",
      );
    });
  });
  ...
  describe("Parameter handling", () => {
    ...
    it("should convert parameter values to strings", async () => {
      ...
      await postsApi.getAll(params);
      expect(mockSecureApiClient.get).toHaveBeenCalledWith(
        "/posts?page=1&limit=10&active=true",
      );
    });
  });
});
